...
impl CurrencyConversion {
    ...
    /// update the conversion rates using the ECB
    pub fn update_conversion_rates() -> Result<(), Box<dyn Error>> {
        // ToDo: use https://www.ecb.europa.eu/stats/eurofxref/eurofxref-daily.xml
        // to retrieve most current exchange rates (EUR = 1.0) and save them
        Ok(())
    }

    /// convert the passed value to the equivalent of the passed to currency
    pub fn convert_price_to(value: f64, from: &SupportedCurrency, to: &SupportedCurrency) -> f64 {
        // ToDo: use the currently not implemented yet exchange rates to calculate the proper
        // value of the passed value
        0.0
    }
}
